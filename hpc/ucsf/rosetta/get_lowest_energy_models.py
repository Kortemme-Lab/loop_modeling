#!/usr/bin/env python2
'''Get the lowest energy models of a data set. Save
the lowest models into a new directory in that dataset
...
if __name__ == '__main__':
    ...
    pdb_to_ids = {}
    pdb_to_lowest_score = {}
    pdb_to_lowest_score_model_id = {}

    for i, b_input in enumerate(benchmark_define_dict['input_pdbs']):
        pdb = os.path.basename(b_input.pdb_path)[:4]
        pdb_to_ids[pdb] = i
        pdb_to_lowest_score[pdb] = float('inf')
        pdb_to_lowest_score_model_id[pdb] = 0

    # Find the lowest scoring models

    for model in results:
        pdb, model_id, score = model[0], model[1], model[3]
        
        if score < pdb_to_lowest_score[pdb]:
            pdb_to_lowest_score[pdb] = score
            pdb_to_lowest_score_model_id[pdb] = model_id

    # Save the lowest energy model to a new directory

    lowest_score_model_dir = os.path.join('data', benchmark_id, 'lowest_score_models')
    if not os.path.exists(lowest_score_model_dir):
        os.mkdir(lowest_score_model_dir)

    num_total_proteins = len(pdb_to_ids.keys())

    for pdb in pdb_to_ids.keys():
        global_model_id = num_total_proteins * pdb_to_lowest_score_model_id[pdb] + pdb_to_ids[pdb]
       
        shutil.copy(os.path.join('data', benchmark_id, 'structures', '{0}_{1}_0001.pdb.gz'.format(global_model_id, pdb)),
                lowest_score_model_dir)
